const { isNotExists } = require('../src')

const variants = [
  { output: true,  input: undefined },
  { output: true,  input: null },
  { output: true,  input: NaN },
  // booleans
  { output: false, input: true },
  { output: false, input: false },
  { output: false, input: new Boolean(), desc: 'new Boolean()' },
  // numbers
  { output: false, input: 0 },
...
variants.forEach((variant, i) => {
  ...
  test(description, () => expect(isNotExists(input)).toBe(output))
})
